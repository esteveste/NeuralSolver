...
class MazeImitationDataset(torch.utils.data.Dataset):
    """This is a dataset class for mazes.
    padding and cropping is done correctly within this class for small and large mazes.
    """
    ...
    def __init__(self,
        ...
        self.folder_name = f"maze_imitation_data_{'train' if self.train else 'test'}_{size}"
        url = f"https://cs.umd.edu/~tomg/download/Easy_to_Hard_Datav2/" \
              f"{self.folder_name}.tar.gz"
...
def get_problem_dataset(problem_name):

    if 'prefix' in problem_name:

        # this is an adapter for mazes datasets...
        class CustomPrefix(torch.utils.data.Dataset):
            base_folder = "prefix_sums_data"
            prob_name = problem_name
            # url = "https://cs.umd.edu/~tomg/download/Easy_to_Hard_Datav2/prefix_sums_data.tar.gz"
            lengths = list(range(16, 65)) + [72] + [128] + [256] + [512]
            download_list = [f"{problem_name}_{l}_data.pth" for l in lengths] + [f"{problem_name}_{l}_targets.pth" for l in lengths]

            def __init__(self, root: str, size: int = 32, download: bool = True,train:bool=None):
                num_bits = size
                ...
                inputs_path = os.path.join(root, self.base_folder, f"{problem_name}_{num_bits}_data.pth")
                targets_path = os.path.join(root, self.base_folder, f"{problem_name}_{num_bits}_targets.pth")
                self.inputs = torch.load(inputs_path).unsqueeze(1) - 0.5
                self.targets = torch.load(targets_path).long()
        ...
        class CustomDataset(torch.utils.data.Dataset):
            ...
            def __init__(self,
                ...
                self.folder_name = f"{problem_name}_data_{'train' if self.train else 'test'}_{size}"
                url = f"https://cs.umd.edu/~tomg/download/Easy_to_Hard_Datav2/" \
                    f"{self.folder_name}.tar.gz"
                ...
                print(f"Loading object of size of size {size} x {size}.")
            ...
            def download(self, url) -> None:
                if self._check_integrity():
                    ...
                    raise RuntimeError(f"Files not found. {self.folder_name}")
                path = download_url(url, self.root)
                extract_zip(path, self.root)
                os.unlink(path)
