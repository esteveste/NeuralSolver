...
class ConvLSTMCell(nn.Module):
    ...
    def forward_input(self, input_tensor):
        """
        This allows for efficient caching of recall feature, for each iteration
        """
    ...
    def forward(self, i2h, cur_state=None):
        ...
        if self.ln_preact:
            h2h = self.ln_h2h(h2h)
        combined_conv = i2h + h2h  # use previous cached input
        ...
        if self.conv_dim == 2:
            ...
            return (
                torch.zeros(
                    ...
                    height,
                    width,
                    device=self.conv_i2h.weight.device,
                ),
                torch.zeros(
                    batch_size,
                    self.hidden_dim,
    ...
    def sample_mask(self, device="cpu"):
        ...
        self.mask = torch.bernoulli(
            torch.empty(1, self.hidden_size, 1, 1).fill_(keep)
        ).to(device)
...
class NeuralSolverClass(nn.Module):
    ...
    def __init__(
        ...
        _dropout_gal2=0,
        norm_affine=True,
        lstm_class=ConvLSTMCell,
        conv_dim: int = 2,
        use_pooling=True,
        use_AvgPool=False,
        use_smaller_head=False,
        **kwargs
    ):
        super().__init__()

        self.name = "NeuralSolverClass"
        ...
        if conv_dim == 2:
            if use_smaller_head:
                head_conv1 = conv_class(
                    width, 32, kernel_size=3, stride=1, padding=1, bias=bias
                )
                head_conv2 = conv_class(
                    32, 8, kernel_size=3, stride=1, padding=1, bias=bias
                )
                head_conv3 = conv_class(
                ...
                head_conv1 = conv_class(
                    width, width, kernel_size=3, stride=1, padding=1, bias=bias
                )
                head_conv2 = conv_class(
                    width, width, kernel_size=3, stride=1, padding=1, bias=bias
                )
                head_conv3 = conv_class(
                    width, output_size, kernel_size=3, stride=1, padding=1, bias=bias
                )
        elif conv_dim == 1:
            head_conv1 = conv_class(
                width, width, kernel_size=3, stride=1, padding=1, bias=bias
            ...
            head_conv2 = conv_class(
                width, int(width / 2), kernel_size=3, stride=1, padding=1, bias=bias
            )
            head_conv3 = conv_class(
                int(width / 2), 2, kernel_size=3, stride=1, padding=1, bias=bias
            )
    ...
    def forward(self, x, iters_to_do, return_all_outputs=False, **kwargs):
        if self.use_pooling:
            ...
            if len(x.shape) == 4:
                all_outputs = torch.zeros(
                    (x.size(0), iters_to_do, self.output_size, x.size(2), x.size(3))
                ).to(x.device)
            elif len(x.shape) == 3:
                all_outputs = torch.zeros(
                    (x.size(0), iters_to_do, self.output_size, x.size(2))
                ).to(x.device)
            else:
                assert False, "not implemented"
...
def neuralsolver_1l_sgal04_py03_2d_out4_avgpool(width, **kwargs):
...
def neuralsolver_1l_sgal04_py03_2d_out10_avgpool(width, **kwargs):
    return NeuralSolverClass(
        width=width,
        output_size=10,
        in_channels=kwargs["in_channels"],
...
def neuralsolver_1l_2d_out4_avgpool(width, **kwargs):
    return NeuralSolverClass(
        width=width,
        output_size=4,
        in_channels=kwargs["in_channels"],
...
def neuralsolver_1l_2d_out3_avgpool(width, **kwargs):
    return NeuralSolverClass(
        width=width,
        output_size=3,
        in_channels=kwargs["in_channels"],
...
def neuralsolver_1l_sgal04_py03_2d_out3_avgpool(width, **kwargs):
    return NeuralSolverClass(
        width=width,
        output_size=3,
        in_channels=kwargs["in_channels"],
...
def neuralsolver_1l_sgal04_py03_2d_out4_maxpool(width, **kwargs):
    return NeuralSolverClass(
        width=width,
        output_size=4,
        in_channels=kwargs["in_channels"],
...
def neuralsolver_1l_sgal04_py03_2d_out3_maxpool(width, **kwargs):
    return NeuralSolverClass(
        width=width,
        output_size=3,
        in_channels=kwargs["in_channels"],
...
def neuralsolver_1l_2d_out4_maxpool(width, **kwargs):
    return NeuralSolverClass(
        width=width,
        output_size=4,
        in_channels=kwargs["in_channels"],
...
def neuralsolver_1l_2d_out3_maxpool(width, **kwargs):
    return NeuralSolverClass(
        width=width,
        output_size=3,
        in_channels=kwargs["in_channels"],
