...
class FeedForwardNet1D(nn.Module):
    ...
    def forward(self, x, iters_to_do, interim_thought=None, iters_elapsed=0, **kwargs):
        ...
        if iters_to_do > self.iters:
            ...
            all_outputs[:, self.iters:] = out.unsqueeze(1).repeat(1, iters_to_do - self.iters, 1, 1)
...
class FeedForwardNet1DReduceMaxPool(nn.Module):
    """Modified Residual Network model class"""

    def __init__(self, block, num_blocks, width, recall,output_size, max_iters=8, group_norm=False):
        super().__init__()
        ...
        head_conv2 = nn.Conv1d(width, width, kernel_size=3, stride=1, padding=1, bias=False)
        head_conv3 = nn.Conv1d(width, 2*output_size, kernel_size=3, stride=1, padding=1, bias=False)

        self.iters = max_iters
        self.projection = nn.Sequential(proj_conv, nn.ReLU())
        self.head = nn.Sequential(head_conv1, nn.ReLU(), 
                                head_pool,
                                head_conv2, nn.ReLU(), head_conv3)
...
def feedforward_net_1d_out2(width, **kwargs):
    return FeedForwardNet1DReduceMaxPool(BasicBlock, [2], width, output_size=2, recall=False, max_iters=kwargs["max_iters"])


def feedforward_net_recall_1d_out2(width, **kwargs):
    return FeedForwardNet1DReduceMaxPool(BasicBlock, [2], width, output_size=2,recall=True, max_iters=kwargs["max_iters"])
