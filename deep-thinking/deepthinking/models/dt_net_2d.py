...
class DTNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        proj_conv = nn.Conv2d(
            in_channels, width, kernel_size=3, stride=1, padding=1, bias=bias
        )

        conv_recall = nn.Conv2d(
            width + in_channels, width, kernel_size=3, stride=1, padding=1, bias=bias
        )
        ...
        head_conv1 = nn.Conv2d(width, 32, kernel_size=3, stride=1, padding=1, bias=bias)
        head_conv2 = nn.Conv2d(32, 8, kernel_size=3, stride=1, padding=1, bias=bias)
        head_conv3 = nn.Conv2d(8, 2, kernel_size=3, stride=1, padding=1, bias=bias)

        self.projection = nn.Sequential(proj_conv, nn.ReLU())
        self.recur_block = nn.Sequential(*recur_layers)
        self.head = nn.Sequential(
            head_conv1, nn.ReLU(), head_conv2, nn.ReLU(), head_conv3
        )
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for strd in strides:
            layers.append(
                block(
                    self.width, planes, strd, group_norm=self.group_norm, bias=self.bias
                )
            )
    ...
    def forward(
        self, x, iters_to_do, interim_thought=None, return_all_outputs=False, **kwargs
    ):
        ...
        all_outputs = torch.zeros((x.size(0), iters_to_do, 2, x.size(2), x.size(3))).to(
            x.device
        )
...
def dt_net_2d(width, **kwargs):
    return DTNet(
        BasicBlock, [2], width=width, in_channels=kwargs["in_channels"], recall=False
    )


def dt_net_recall_2d(width, **kwargs):
    return DTNet(
        BasicBlock, [2], width=width, in_channels=kwargs["in_channels"], recall=True
    )
...
def dt_net_recall_2d_1block(width, **kwargs):
    return DTNet(
        BasicBlock, [1], width=width, in_channels=kwargs["in_channels"], recall=True
    )
...
class DTNetReduceMaxPool(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        conv_recall = nn.Conv2d(
            width + in_channels, width, kernel_size=3, stride=1, padding=1, bias=False
        )
        ...
        head_conv1 = nn.Conv2d(
            width, width, kernel_size=3, stride=1, padding=1, bias=False
        )
        head_conv2 = nn.Conv2d(
            width, 32, kernel_size=3, stride=1, padding=1, bias=False
        )
        head_conv3 = nn.Conv2d(
            32, output_size, kernel_size=3, stride=1, padding=1, bias=False
        )

        self.projection = nn.Sequential(proj_conv, nn.ReLU())
        self.recur_block = nn.Sequential(*recur_layers)
        self.head = nn.Sequential(
            head_conv1, nn.ReLU(), head_pool, head_conv2, nn.ReLU(), head_conv3
        )
    ...
    def forward(
        ...
        all_outputs = torch.zeros((x.size(0), iters_to_do, self.output_size)).to(
            x.device
        )
...
class FeedForwardNetAvgPoolEnd(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(max_iters):
            ...
            for j in range(len(num_blocks)):
                internal_block.append(
                    self._make_layer(block, width, num_blocks[j], stride=1)
                )
        ...
        if paper_out_head:
            ...
            head_conv3 = nn.Conv2d(
                64, output_size, kernel_size=3, stride=1, padding=1, bias=False
            )

        self.iters = max_iters
        self.projection = nn.Sequential(proj_conv, nn.ReLU())
        self.head = nn.Sequential(
            head_conv1, nn.ReLU(), head_conv2, nn.ReLU(), head_conv3, head_pool
        )
    ...
    def forward(self, x, iters_to_do, interim_thought=None, iters_elapsed=0, **kwargs):
        ...
        for i, layer in enumerate(
            self.feedforward_layers[iters_elapsed : iters_elapsed + iters_to_do]
        ):
            if self.recall:
                interim_thought = torch.cat([interim_thought, x], 1)
        ...
        if iters_to_do > self.iters:
            ...
            all_outputs[:, self.iters :] = out.unsqueeze(1).repeat(
                1, iters_to_do - self.iters, 1
            )
...
def feedforward_net_recall_2d_out4_avgpool(width, **kwargs):
    return FeedForwardNetAvgPoolEnd(
        BasicBlock,
        [2],
        width,
        output_size=4,
...
class DTNetPool(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        head_conv2 = nn.Conv2d(32, 8, kernel_size=3, stride=1, padding=1, bias=bias)
        head_conv3 = nn.Conv2d(
            8, output_size, kernel_size=3, stride=1, padding=1, bias=bias
        )
...
class DTNetRandom(nn.Module):
    ...
    def forward(
        ...
        all_outputs = torch.rand(
            (x.size(0), iters_to_do, self.output_size), requires_grad=True
        ).to(x.device)
